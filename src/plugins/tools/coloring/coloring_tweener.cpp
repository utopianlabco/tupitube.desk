...
QWidget *ColoringTweener::configurator()
{
    if (!configPanel) {
        ...
        configPanel = new ColoringConfigurator;
...
void ColoringTweener::removeTweenFromProject(const QString &name)
{
    ...
    if (!frame) {
        #ifdef TUP_DEBUG
            qDebug() << "[Coloring Tweener::removeTweenFromProject()] - Invalid tween frame ->"
    ...
    foreach (QGraphicsItem *item, tweenItems) {
        ...
        if (TupSvgItem *svg = qgraphicsitem_cast<TupSvgItem *>(item)) {
            ...
            if (objectId.isEmpty()) {
                #ifdef TUP_DEBUG
                    qWarning() << "[Coloring Tweener::removeTweenFromProject()] - "
