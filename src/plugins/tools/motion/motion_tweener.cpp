...
void MotionTweener::updateScene(TupGraphicsScene *scene)
{
    ...
    if (mode == TupToolPlugin::Edit) {
        ...
        // Snapshot recovery replaces the project model in place. If a local
        // optimistic RemoveTween was rejected, currentTween is NULL after the
...
void MotionTweener::removeTweenFromProject(const QString &name)
{
    ...
    if (!frame) {
        #ifdef TUP_DEBUG
            qDebug() << "[Motion Tweener::removeTweenFromProject()] - Invalid tween frame ->"
    ...
    foreach (QGraphicsItem *item, tweenItems) {
        ...
        if (TupSvgItem *svg = qgraphicsitem_cast<TupSvgItem *>(item)) {
            ...
            if (objectId.isEmpty()) {
                #ifdef TUP_DEBUG
                    qWarning() << "[Motion Tweener::removeTweenFromProject()] - "
