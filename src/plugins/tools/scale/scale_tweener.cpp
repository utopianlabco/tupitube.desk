...
ScaleTweener::ScaleTweener() : TupToolPlugin()
{
    setupActions();
...
void ScaleTweener::init(TupGraphicsScene *gScene)
{
    #ifdef TUP_DEBUG
        qDebug() << "[Scale Tweener::init()]";
    #endif
...
void ScaleTweener::updateStartPoint(int index)
{
...
QList<TAction::ActionId> ScaleTweener::keys() const
{
...
void ScaleTweener::press(const TupInputDeviceInformation *input, TupBrushManager *brushManager, TupGraphicsScene *gScene)
{
    #ifdef TUP_DEBUG
        qDebug() << "[Scale Tweener::press()]";
    #endif
...
void ScaleTweener::move(const TupInputDeviceInformation *input, TupBrushManager *brushManager, TupGraphicsScene *gScene)
{
    Q_UNUSED(input)
    Q_UNUSED(brushManager)
    Q_UNUSED(gScene)
...
void ScaleTweener::release(const TupInputDeviceInformation *input, TupBrushManager *brushManager, TupGraphicsScene *gScene)
{
    #ifdef TUP_DEBUG
        qDebug() << "[Scale Tweener::release()]";
    #endif
    ...
    if (gScene->currentFrameIndex() == initFrame) {
        if (editMode == TupToolPlugin::Selection) {
            ...
            if (gScene->selectedItems().size() > 0) {
                #ifdef TUP_DEBUG
                    qDebug() << "[Scale Tweener::release()] - selection size -> " << gScene->selectedItems().size();
                #endif
...
QMap<TAction::ActionId, TAction *> ScaleTweener::actions() const
{
    return scaleActions;
}

TAction * ScaleTweener::getAction(TAction::ActionId toolId)
{
...
int ScaleTweener::toolType() const
{
...
QWidget *ScaleTweener::configurator()
{
    if (!configPanel) {
        mode = TupToolPlugin::View;
...
void ScaleTweener::aboutToChangeScene(TupGraphicsScene *)
{
}

/* This method is called when this plugin is off */

void ScaleTweener::aboutToChangeTool()
{
    if (editMode == TupToolPlugin::Selection) {
        clearSelection();
        disableSelection();
...
void ScaleTweener::setupActions()
{
...
void ScaleTweener::saveConfig()
{
}

/* This method updates the workspace when the plugin changes the scene */

void ScaleTweener::updateScene(TupGraphicsScene *gScene)
{ 
    #ifdef TUP_DEBUG
        qDebug() << "[Scale Tweener::updateScene()]";
    #endif
...
void ScaleTweener::clearSelection()
{
    if (objects.size() > 0) {
        foreach (QGraphicsItem *item, objects) {
            if (item->isSelected())
...
void ScaleTweener::disableSelection()
{
...
void ScaleTweener::setSelection()
{
...
void ScaleTweener::setPropertiesMode()
{
...
void ScaleTweener::applyTween()
{
    ...
    if (tweenAlreadyExists) {
        ...
        if (tweenId.isEmpty()) {
            #ifdef TUP_DEBUG
                qWarning() << "[Scale Tweener::applyTween()] - Existing tween is missing tween_id ->" << sourceTweenName;
            #endif
        ...
        foreach (QGraphicsItem *item, objects) {
            ...
            TupProjectRequest request = TupRequestBuilder::createItemRequest(
                                        ...
                                        configPanel->tweenToXml(initScene, initLayer, initFrame, tweenId, pos,
                                                                initialXScaleFactor, initialYScaleFactor), QByteArray(), QString(), QString(), objectId);
...
QTransform ScaleTweener::initialStep()
{
...
void ScaleTweener::updateMode(TupToolPlugin::Mode currentMode)
{
    mode = currentMode;
    ...
    if (mode == TupToolPlugin::Edit) {
        if (currentTween) {
            ...
            #ifdef TUP_DEBUG
                qDebug() << "[Scale Tweener::updateMode()] - Current tween pointer is NULL!";
            #endif
        }
    }
}
...
void ScaleTweener::layerResponse(const TupLayerResponse *event)
{
    if (event->getAction() == TupProjectRequest::Remove)
        init(scene);
}

void ScaleTweener::frameResponse(const TupFrameResponse *event)
{
